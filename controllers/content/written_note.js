    ...
    Course.findOne({_id:content_exists.course_id},(err,course_exists)=>{
      ...
      Marks.remove({content_id:content_exists._id},(err)=>{
        ...
        content_exists.remove((err)=>{
          if(err) return log_err(err,false,req,res);
          else if(content_exists && (content_exists.type ==req.app.locals.type.W_Note 
            || content_exists.type ==req.app.locals.type.Uploaded_Assessment)){
          ...
          async.parallel([
            (callback)=>{
              ...
              .find({course_id:req.params.course_id,type:{$gte:3,$lte:6}},
                {source_question:0,course_id:0,school_id:0,unit_id:0,owner_URN:0,answers:0,time:0,
